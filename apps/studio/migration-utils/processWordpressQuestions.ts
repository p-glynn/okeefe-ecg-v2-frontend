...
async function processQuestions(inputType: InputType, fullOrTest: string) {
    ...
    const output = data.map((question: Question) => {
        const {id, question_title, explanation, secondary_text, ...restProps} = question;
            ...
            if (mediaProperties.includes(key)) {
                ...
                if (outputType === 'qbank') {
                    if (key.includes('correct_answer')) {
                        const correctAnswers = value as unknown as QBankAnswer[];
